...
rule frips_plot:
    ...
    conda: "../envs/frips/frips.yaml"
    shell:
        "CHIPS/modules/scripts/frips_figure.py -f {input} -o {output}"
...
rule frips_collectPbc:
    ...
    shell:
        "CHIPS/modules/scripts/frips_collectPBC.py {params.files} > {output} " #2>>{log}"
...
rule frips_collectNonChrMStats:
    ...
    shell:
        "CHIPS/modules/scripts/frips_collectNonChrM.py {params.files} > {output} " #2>>{log}"
...
rule frips_getFripStats:
    ...
    shell:
        "CHIPS/modules/scripts/frips_getFrips.py {params.files} -o {output} " #2>>{log}"
